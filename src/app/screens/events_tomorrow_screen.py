...
from src.services.structure_parser import get_events_tomorrow
...
class EventsTomorrowScreen(BaseScreen):
    """Screen displaying tomorrow's events."""
    
    def __init__(self):
        super().__init__(name="Events Tomorrow")
        self._events: List[Dict[str, Any]] = []
    
    def get_data(self) -> dict:
        """Fetch tomorrow's events from Google Calendar."""
        try:
            self._events = get_events_tomorrow()
            logger.info("Fetched %d events for tomorrow", len(self._events))
        except Exception as e:
            logger.exception("Failed to fetch tomorrow's events: %s", e)
            self._events = []
        return {"events": self._events}
    
    def render(self) -> Image.Image:
        """Render tomorrow's events screen."""
        epd = get_epd()
        ...
        # Header
        draw.text((5, y_offset), "TOMORROW'S EVENTS", fill=0, font=font_large)
        y_offset += line_height + 2
        
        if not self._events:
            draw.text((5, y_offset), "No events tomorrow", fill=0, font=font_small)
            return img
    ...
    def on_enter(self) -> None:
        ...
        logger.info("Entering Events Tomorrow screen")
        self.get_data()
